...
const copyImageButton = document.getElementById("btnCopyImage");
...
const timelineLocaleControl = timelineLocale.closest("label");
timelineLocaleControl.classList.add("theme-timeline-locale");
timelineLocaleControl.remove();
...
const DEFAULT_THEME_APPEARANCE = { timelineYear: "@primaryStrong", timelineYearAlternate: "@primary", timelineQuarter: "@primaryStrong", timelineQuarterAlternate: "@primaryStrong", timelineMonth: "@primary", timelineMonthAlternate: "@primary", timelineWeek: "@primarySoft", timelineWeekAlternate: "@primarySoft", timelineDay: "@neutralSoft", timelineDayAlternate: "@neutralSoft", todayLine: "@danger", dependencyIncoming: "@accent", dependencyOutgoing: "@primaryStrong", dependencyNeutral: "@neutral" };
const DEFAULT_THEME_GRID = { primary: { color: "@border", width: 1.2, style: "solid" }, secondary: { color: "@neutralSoft", width: 0.6, style: "dotted" } };
const DEFAULT_TIMELINE_FONT_SIZES = { year: 12, quarter: 13, month: 18, week: 11, day: 10 };
const DATE_VALUE_FORMATS = [["day", "7"], ["dayPadded", "07"], ["monthNumber", "9"], ["monthPadded", "09"], ["monthShort", "sept."], ["monthLong", "septembre"], ["quarter", "T3"], ["year", "2026"], ["yearShort", "26"]];
const TIMELINE_FORMAT_CATALOG = {
  year: [{ type: "year", label: "Année", formats: [["plain", "2026"], ["short", "26"]] }],
  quarter: [{ type: "quarter", label: "Numéro du trimestre", formats: [["plain", "3"]] }, { type: "year", label: "Année", formats: [["plain", "2026"], ["short", "26"]] }],
  month: [{ type: "monthNumber", label: "Numéro du mois", formats: [["plain", "9"], ["padded", "09"]] }, { type: "monthName", label: "Nom du mois", formats: [["short", "sept."], ["long", "septembre"]] }, { type: "year", label: "Année", formats: [["plain", "2026"], ["short", "26"]] }, { type: "startDate", label: "Date de début", formats: DATE_VALUE_FORMATS }, { type: "endDate", label: "Date de fin", formats: DATE_VALUE_FORMATS }],
  week: [{ type: "weekNumber", label: "Numéro de semaine", formats: [["plain", "37"]] }, { type: "weekStart", label: "Premier jour de la semaine", formats: DATE_VALUE_FORMATS }, { type: "weekFirstWorkday", label: "Premier jour ouvrable de la semaine", formats: DATE_VALUE_FORMATS }, { type: "weekEnd", label: "Dernier jour de la semaine", formats: DATE_VALUE_FORMATS }, { type: "weekLastWorkday", label: "Dernier jour ouvrable de la semaine", formats: DATE_VALUE_FORMATS }],
  day: [{ type: "day", label: "Jour du mois", formats: [["plain", "7"], ["padded", "07"]] }, { type: "weekday", label: "Jour de la semaine", formats: [["narrow", "L"], ["short", "lun."], ["long", "lundi"]] }, { type: "monthName", label: "Mois", formats: [["plain", "9"], ["padded", "09"], ["short", "sept."], ["long", "septembre"]] }, { type: "year", label: "Année", formats: [["plain", "2026"], ["short", "26"]] }, { type: "startDate", label: "Date", formats: DATE_VALUE_FORMATS }]
};
const TIMELINE_FORMAT_PRESETS = {
  week: [["S37", [{ type: "text", value: "S" }, { type: "weekNumber", format: "plain" }]], ["07/09", [{ type: "weekStart", format: "dayPadded" }, { type: "text", value: "/" }, { type: "weekStart", format: "monthPadded" }]], ["7-11", [{ type: "weekStart", format: "day" }, { type: "text", value: "-" }, { type: "weekEnd", format: "day" }]], ["7 sept.", [{ type: "weekStart", format: "day" }, { type: "text", value: " " }, { type: "weekStart", format: "monthShort" }]]],
  month: [["sept. 2026", [{ type: "monthName", format: "short" }, { type: "text", value: " " }, { type: "year", format: "plain" }]]],
  day: [["07/09", [{ type: "startDate", format: "dayPadded" }, { type: "text", value: "/" }, { type: "startDate", format: "monthPadded" }]], ["7 sept.", [{ type: "startDate", format: "day" }, { type: "text", value: " " }, { type: "startDate", format: "monthShort" }]]]
};
const timelineLevels = [
  { key: "year", label: "Années", toggle: document.getElementById("toggleYearTimeline"), h: 20, fillRole: "timelineYear", altRole: "timelineYearAlternate", cls: "year-label" },
  { key: "quarter", label: "Trimestres", toggle: document.getElementById("toggleQuarterTimeline"), h: 24, fillRole: "timelineQuarter", altRole: "timelineQuarterAlternate", cls: "quarter-label" },
  { key: "month", label: "Mois", toggle: document.getElementById("toggleMonthTimeline"), h: 40, fillRole: "timelineMonth", altRole: "timelineMonthAlternate", cls: "month-label" },
  { key: "week", label: "Semaines", toggle: document.getElementById("toggleWeekTimeline"), h: 22, fillRole: "timelineWeek", altRole: "timelineWeekAlternate", cls: "week-label" },
  { key: "day", label: "Jours", toggle: document.getElementById("toggleDayTimeline"), h: 20, fillRole: "timelineDay", altRole: "timelineDayAlternate", cls: "day-label" }
];
const defaultTimelineSettings = () => ({
  levels: { year: true, quarter: false, month: true, week: false, day: false },
  gridPrimaryLevel: "month",
  gridSecondaryLevel: "",
  showTodayLine: true,
  todayLineIncludesTimeline: true,
...
function emptyPlanning() {
  ...
  return {
    ...
    layout: { width: 1500, left: 86, right: 16, topMonths: 8, yearHeight: 20, quarterHeight: 24, monthHeight: 40, weekHeight: 22, dayHeight: 20, timelineTop: 58, lanesTop: 160, laneGap: 5, lanePaddingBottom: 10, defaultItemHeight: 30 },
    monthLocale: "fr-FR", items: [], milestones: [], lanes: [], overlays: [], timeline: defaultTimelineSettings(), workCalendar: defaultWorkCalendar(),
    theme: { preset: "ocean", colors: clone(THEME_PRESETS.ocean.colors), appearance: clone(DEFAULT_THEME_APPEARANCE), timeline: clone(DEFAULT_TIMELINE_FONT_SIZES), timelineFormats: {} },
...
function visibleLevels() { return timelineLevels.filter(v => v.toggle.checked).map(v => ({ ...v, height: planningData.layout[`${v.key}Height`] ?? v.h, fontSize: planningData.theme.timeline[v.key] })); }
...
function defaultTimelineFormat(level) { const language = new Intl.Locale(timelineLocaleValue()).language; if (level === "year") return [{ type: "year", format: "plain" }]; if (level === "quarter") return [{ type: "text", value: language === "fr" ? "T" : "Q" }, { type: "quarter", format: "plain" }]; if (level === "week") return [{ type: "text", value: language === "fr" ? "S" : "W" }, { type: "weekNumber", format: "plain" }]; if (level === "day") return [{ type: "day", format: "plain" }]; return [{ type: "monthName", format: "short" }]; }
function timelineFormatParts(level) { return planningData.theme.timelineFormats?.[level] || defaultTimelineFormat(level); }
function timelineTokenValue(type, format, start, end, level) {
  const locale = timelineLocaleValue(), numeric = value => String(value), padded = value => String(value).padStart(2, "0");
  const month = (date, style) => new Intl.DateTimeFormat(locale, { month: style }).format(date).replace(/^./, char => char.toUpperCase());
  const date = (value, style) => style === "day" ? numeric(value.getDate()) : style === "dayPadded" ? padded(value.getDate()) : style === "numeric" ? `${padded(value.getDate())}/${padded(value.getMonth() + 1)}` : style === "monthNumber" ? numeric(value.getMonth() + 1) : style === "monthPadded" ? padded(value.getMonth() + 1) : style === "monthShort" ? month(value, "short") : style === "monthLong" ? month(value, "long") : style === "year" ? String(value.getFullYear()) : style === "yearShort" ? String(value.getFullYear()).slice(-2) : style === "quarter" ? quarterLabel(value) : `${numeric(value.getDate())} ${month(value, style === "long" ? "long" : "short")}`;
  const workingDay = (fromStart) => { const direction = fromStart ? 1 : -1, cursor = new Date((fromStart ? start : end).getTime()), daysOff = new Set((planningData.workCalendar.daysOff || []).map(day => day.date)); while (weeklyDayOff(dateIso(cursor)) || daysOff.has(dateIso(cursor))) cursor.setDate(cursor.getDate() + direction); return cursor; };
  const target = type === "weekEnd" ? end : type === "weekFirstWorkday" ? workingDay(true) : type === "weekLastWorkday" ? workingDay(false) : type === "weekStart" ? start : type.startsWith("end") ? end : start;
  if (type === "year") return format === "short" ? String(start.getFullYear()).slice(-2) : String(start.getFullYear());
  if (type === "quarter") return String(Math.floor(start.getMonth() / 3) + 1);
  if (type === "weekNumber") return String(week(start));
  if (type === "monthNumber") return format === "padded" ? padded(start.getMonth() + 1) : numeric(start.getMonth() + 1);
  if (type === "monthName" || type === "startMonth" || type === "endMonth") return ["plain", "padded"].includes(format) ? (format === "padded" ? padded(target.getMonth() + 1) : numeric(target.getMonth() + 1)) : month(target, format === "long" ? "long" : "short");
  if (type === "day" || type === "startDay" || type === "endDay") return format === "padded" ? padded(target.getDate()) : numeric(target.getDate());
  if (type === "weekday") return new Intl.DateTimeFormat(locale, { weekday: format === "long" ? "long" : format === "narrow" ? "narrow" : "short" }).format(start).replace(/^./, char => char.toUpperCase());
  if (["startDate", "endDate", "weekStart", "weekFirstWorkday", "weekEnd", "weekLastWorkday"].includes(type)) return date(target, format);
  return level === "year" ? String(start.getFullYear()) : "";
}
function timelineLabelParts(level, start, next) {
  const custom = timelineFormatParts(level), end = new Date(next.getFullYear(), next.getMonth(), next.getDate() - 1);
  if (custom?.length) return custom.map(part => ({ ...part, value: part.type === "text" ? part.value : timelineTokenValue(part.type, part.format || "plain", start, end, level) }));
  const label = level === "year" ? String(start.getFullYear()) : level === "quarter" ? quarterLabel(start) : level === "week" ? weekLabel(start) : level === "day" ? String(start.getDate()) : new Intl.DateTimeFormat(timelineLocaleValue(), { month: "short" }).format(start).replace(/^./, c => c.toUpperCase());
  return [{ type: "text", value: label }];
}
function periods(level) { const start = new Date(`${planningData.range.start}T00:00:00`), end = new Date(`${planningData.range.end}T00:00:00`), output = []; let d, next; if (level === "year") { d = new Date(start.getFullYear(), 0, 1); next = v => new Date(v.getFullYear() + 1, 0, 1); } else if (level === "quarter") { d = new Date(start.getFullYear(), Math.floor(start.getMonth() / 3) * 3, 1); next = v => new Date(v.getFullYear(), v.getMonth() + 3, 1); } else if (level === "week") { d = new Date(start); d.setDate(d.getDate() - ((d.getDay() + 6) % 7)); next = v => new Date(v.getFullYear(), v.getMonth(), v.getDate() + 7); } else if (level === "day") { d = new Date(start); next = v => new Date(v.getFullYear(), v.getMonth(), v.getDate() + 1); } else { d = new Date(start.getFullYear(), start.getMonth(), 1); next = v => new Date(v.getFullYear(), v.getMonth() + 1, 1); } const boundedDate = value => { const iso = dateIso(value); return iso < planningData.range.start ? planningData.range.start : iso > planningData.range.end ? rangeEndBoundary() : iso; }; while (d <= end) { const following = next(d); output.push([timelineLabelParts(level, d, following), boundedDate(d)]); d = following; } output.push(["", boundedDate(d)]); return output; }
...
function doubleChevron(x1, y, w, h, angle) { const x2 = x1 + w, depth = chevronDepth(w, h, angle, true), interlockGap = 2, start = x1 + interlockGap; return `M ${start - depth} ${y} H ${x2 - depth} L ${x2} ${y + h / 2} L ${x2 - depth} ${y + h} H ${start - depth} L ${start} ${y + h / 2} Z`; }
...
function drawMilestoneVerticalLine(item) {
  ...
  svg.appendChild(el("line", {
    x1: milestoneX(date), y1: item.verticalLineIncludesTimeline ? 0 : geometry.timelineTop, x2: milestoneX(date), y2: geometry.H,
...
function normalise(data) {
  ...
  const requestedFormats = data.theme?.timelineFormats || {};
  const normalisedFormats = {};
  timelineLevels.forEach(level => {
    const available = new Map((TIMELINE_FORMAT_CATALOG[level.key] || []).map(item => [item.type, item]));
    if (!Array.isArray(requestedFormats[level.key])) return;
    const parts = requestedFormats[level.key].flatMap(part => {
      if (!part || typeof part !== "object") return [];
      if (part.type === "text") return typeof part.value === "string" && part.value ? [{ type: "text", value: part.value.slice(0, 200), bold: !!part.bold, italic: !!part.italic }] : [];
      const legacyWeekType = { startDate: "weekStart", startDay: "weekStart", startMonth: "weekStart", endDate: "weekEnd", endDay: "weekEnd", endMonth: "weekEnd" };
      const type = level.key === "week" ? (legacyWeekType[part.type] || part.type) : part.type;
      const format = level.key === "week" && ["startDay", "endDay"].includes(part.type) ? (part.format === "padded" ? "dayPadded" : "day") : level.key === "week" && ["startMonth", "endMonth"].includes(part.type) ? ({ plain: "monthNumber", padded: "monthPadded", short: "monthShort", long: "monthLong" }[part.format] || part.format) : part.format;
      if (["numeric", "short", "long"].includes(format) && ["startDate", "endDate", "weekStart", "weekEnd"].includes(type)) { const fragments = format === "numeric" ? [[type, "dayPadded"], ["text", "/"], [type, "monthPadded"]] : [[type, "day"], ["text", " "], [type, format === "short" ? "monthShort" : "monthLong"]]; return fragments.map(([fragmentType, fragmentFormat]) => fragmentType === "text" ? { type: "text", value: fragmentFormat, bold: !!part.bold, italic: !!part.italic } : { type: fragmentType, format: fragmentFormat, bold: !!part.bold, italic: !!part.italic }); }
      const item = available.get(type);
      if (!item || !item.formats.some(([candidate]) => candidate === format)) return [];
      return [{ type, format, bold: !!part.bold, italic: !!part.italic }];
    });
    if (parts.length) normalisedFormats[level.key] = parts;
  });
  data.theme = { preset: presetId, colors, appearance: { ...DEFAULT_THEME_APPEARANCE, ...appearance }, timeline: { ...DEFAULT_TIMELINE_FONT_SIZES, ...(data.theme?.timeline || {}) }, timelineFormats: normalisedFormats, grid: { primary: { ...DEFAULT_THEME_GRID.primary, ...(data.theme?.grid?.primary || {}), ...(data.theme?.grid?.primary?.color ? {} : legacyGridColor ? { color: legacyGridColor } : {}) }, secondary: { ...DEFAULT_THEME_GRID.secondary, ...(data.theme?.grid?.secondary || {}) } } };
  ...
  timelineLevels.forEach(level => {
    data.theme.timeline[level.key] = Math.max(6, Math.min(48, Number.isFinite(Number(data.theme.timeline[level.key])) ? Number(data.theme.timeline[level.key]) : DEFAULT_TIMELINE_FONT_SIZES[level.key]));
    const heightKey = `${level.key}Height`;
    data.layout[heightKey] = Math.max(12, Math.min(120, Number.isFinite(Number(data.layout[heightKey])) ? Number(data.layout[heightKey]) : level.h));
  });
  ...
  if (!["year", "quarter", "month", "week", "day"].includes(data.timeline.gridPrimaryLevel)) data.timeline.gridPrimaryLevel = defaults.gridPrimaryLevel;
  if (data.timeline.gridSecondaryLevel !== "" && !["year", "quarter", "month", "week", "day"].includes(data.timeline.gridSecondaryLevel)) data.timeline.gridSecondaryLevel = defaults.gridSecondaryLevel;
  ...
  if (typeof data.timeline.todayLineIncludesTimeline !== "boolean") data.timeline.todayLineIncludesTimeline = defaults.todayLineIncludesTimeline;
  ...
  const normaliseMilestone = milestone => {
    ...
    if (typeof milestone.verticalLineIncludesTimeline !== "boolean") milestone.verticalLineIncludesTimeline = true;
...
function pngBlob() { return new Promise((resolve, reject) => { const url = URL.createObjectURL(new Blob([xml()], { type: "image/svg+xml;charset=utf-8" })), image = new Image(); image.onload = () => { const c = document.createElement("canvas"), scale = 2; c.width = geometry.W * scale; c.height = geometry.H * scale; c.getContext("2d").drawImage(image, 0, 0, c.width, c.height); c.toBlob(blob => { URL.revokeObjectURL(url); blob ? resolve(blob) : reject(); }, "image/png"); }; image.onerror = () => { URL.revokeObjectURL(url); reject(); }; image.src = url; }); }
let copyImageFeedbackTimeout;
function confirmImageCopied() { clearTimeout(copyImageFeedbackTimeout); copyImageButton.classList.remove("copy-confirmed"); void copyImageButton.offsetWidth; copyImageButton.classList.add("copy-confirmed"); copyImageFeedbackTimeout = setTimeout(() => copyImageButton.classList.remove("copy-confirmed"), 1100); }
...
function addMilestoneVerticalLineEditor(object) {
  ...
  if (object.showVerticalLine) controls.push(
    ...
    fieldGrid(input("Style de ligne", "lineStyle", object.lineStyle || "solid", "select", [["solid", "Plein"], ["dashed", "Tirets"], ["dotted", "Points"]]), input("Inclure la frise", "verticalLineIncludesTimeline", object.verticalLineIncludesTimeline !== false, "checkbox"))
  );
...
function renderTimelineEditor() {
  ...
  const todayTimeline = input("Inclure la frise", "todayLineIncludesTimeline", planningData.timeline.todayLineIncludesTimeline !== false, "checkbox");
  todayTimeline.querySelector("input").oninput = event => { planningData.timeline.todayLineIncludesTimeline = event.target.checked; markTimelineChange(); render(); };
  card.appendChild(section("Ligne d’aujourd’hui", [fieldGrid(todayControl, todayTimeline)], false));
...
function setThemePreset(id) {
  ...
  planningData.theme = {
    ...
    timeline: clone(planningData.theme.timeline || DEFAULT_TIMELINE_FONT_SIZES),
    grid: clone(planningData.theme.grid || DEFAULT_THEME_GRID),
    timelineFormats: clone(planningData.theme.timelineFormats || {})
  };
  applyThemeToApp();
...
function updateTimelineLevelStyle(level, key, value) {
  if (key === "height") {
    planningData.layout[`${level}Height`] = Math.max(12, Math.min(120, Number(value) || timelineLevels.find(candidate => candidate.key === level)?.h || 20));
  } else if (key === "fontSize") {
    planningData.theme.timeline[level] = Math.max(6, Math.min(48, Number(value) || DEFAULT_TIMELINE_FONT_SIZES[level]));
  } else return;
  isDirty = true; importedVersion = false; status(); render();
}
function updateTimelineFormat(level, parts) { planningData.theme.timelineFormats ||= {}; if (parts?.length) planningData.theme.timelineFormats[level] = parts; else delete planningData.theme.timelineFormats[level]; isDirty = true; importedVersion = false; status(); render(); }
function formatExampleParts(level) { const start = new Date("2026-09-07T12:00:00"), next = level === "week" ? new Date("2026-09-14T12:00:00") : level === "month" ? new Date("2026-10-01T12:00:00") : level === "quarter" ? new Date("2026-10-01T12:00:00") : level === "year" ? new Date("2027-01-01T12:00:00") : new Date("2026-09-08T12:00:00"); return timelineLabelParts(level, start, next); }
function timelineFormatEditor(level) {
  const wrap = document.createElement("div"); wrap.className = "timeline-format-editor";
  const caption = document.createElement("span"); caption.className = "timeline-format-caption"; caption.textContent = "Format";
  const editable = document.createElement("div"); editable.className = "timeline-format-content"; editable.contentEditable = "true"; editable.setAttribute("role", "textbox"); editable.setAttribute("aria-label", "Format du libellé");
  const tokenInfo = type => (TIMELINE_FORMAT_CATALOG[level] || []).find(item => item.type === type);
  const renderContent = () => {
    editable.replaceChildren();
    (timelineFormatParts(level) || []).forEach(part => {
      if (part.type === "text") { const text = document.createElement(part.bold ? "strong" : part.italic ? "em" : "span"); if (part.bold && part.italic) { const italic = document.createElement("em"); italic.textContent = part.value; text.appendChild(italic); } else text.textContent = part.value; editable.appendChild(text); return; }
      const chip = document.createElement("span"); chip.className = "timeline-format-token"; chip.contentEditable = "false"; chip.setAttribute("role", "button"); chip.tabIndex = 0; chip.dataset.type = part.type; chip.dataset.format = part.format; chip.dataset.bold = String(!!part.bold); chip.dataset.italic = String(!!part.italic); chip.textContent = timelineTokenValue(part.type, part.format, new Date("2026-09-07T12:00:00"), new Date("2026-09-13T12:00:00"), level); chip.title = tokenInfo(part.type)?.label || "Valeur dynamique"; if (part.bold) chip.classList.add("is-bold"); if (part.italic) chip.classList.add("is-italic"); chip.onclick = () => openTokenMenu(chip); editable.append(chip, document.createTextNode("\u200B"));
    });
  };
  const readParts = () => {
    const output = [];
    const visit = (node, style = {}) => [...node.childNodes].forEach(child => { if (child.nodeType === Node.TEXT_NODE) { const value = child.nodeValue.replace(/\u200B/g, ""); if (value) output.push({ type: "text", value, ...style }); return; } if (child.dataset?.type) { output.push({ type: child.dataset.type, format: child.dataset.format, bold: child.dataset.bold === "true" || style.bold, italic: child.dataset.italic === "true" || style.italic }); return; } visit(child, { bold: style.bold || ["STRONG", "B"].includes(child.tagName), italic: style.italic || ["EM", "I"].includes(child.tagName) }); });
    visit(editable); return output;
  };
  const save = () => updateTimelineFormat(level, readParts());
  const openTokenMenu = chip => {
    document.querySelectorAll(".timeline-token-menu").forEach(menu => menu.remove()); const info = tokenInfo(chip.dataset.type); if (!info) return;
    const menu = document.createElement("div"); menu.className = "timeline-token-menu"; info.formats.forEach(([format, label]) => { const option = document.createElement("button"); option.type = "button"; option.textContent = label; option.onclick = () => { chip.dataset.format = format; chip.textContent = timelineTokenValue(chip.dataset.type, format, new Date("2026-09-07T12:00:00"), new Date("2026-09-13T12:00:00"), level); menu.remove(); save(); refreshPreview(); }; menu.appendChild(option); }); chip.after(menu); const anchor = chip.getBoundingClientRect(), bounds = menu.getBoundingClientRect(); menu.style.left = `${Math.max(12, Math.min(anchor.left, window.innerWidth - bounds.width - 12))}px`; menu.style.top = `${Math.max(12, anchor.top - bounds.height - 4)}px`;
  };
  editable.oninput = () => save();
  editable.onkeydown = event => { const selection = window.getSelection(); if (!selection?.rangeCount || !selection.getRangeAt(0).collapsed || !["Backspace", "Delete"].includes(event.key)) return; const range = selection.getRangeAt(0), node = range.startContainer, offset = range.startOffset; const sibling = node.nodeType === Node.TEXT_NODE ? (event.key === "Backspace" && offset === 0 ? node.previousSibling : event.key === "Delete" && offset >= node.nodeValue.length ? node.nextSibling : null) : null; if (!sibling?.classList?.contains("timeline-format-token")) return; event.preventDefault(); const spacer = event.key === "Backspace" ? node : sibling.nextSibling; sibling.remove(); if (spacer?.nodeType === Node.TEXT_NODE) { const caret = document.createRange(); caret.setStart(spacer, 0); caret.collapse(true); selection.removeAllRanges(); selection.addRange(caret); } save(); };
  const actions = document.createElement("div"); actions.className = "timeline-format-actions";
  const values = document.createElement("button"); values.type = "button"; values.textContent = "+ Valeur"; const valueMenu = document.createElement("div"); valueMenu.className = "timeline-value-menu"; valueMenu.hidden = true;
  (TIMELINE_FORMAT_CATALOG[level] || []).forEach(item => { const option = document.createElement("button"); option.type = "button"; option.textContent = item.label; option.onclick = () => { const [format] = item.formats[0]; const chip = document.createElement("span"); chip.className = "timeline-format-token"; chip.contentEditable = "false"; chip.setAttribute("role", "button"); chip.tabIndex = 0; chip.dataset.type = item.type; chip.dataset.format = format; chip.dataset.bold = "false"; chip.dataset.italic = "false"; chip.textContent = timelineTokenValue(item.type, format, new Date("2026-09-07T12:00:00"), new Date("2026-09-13T12:00:00"), level); chip.title = item.label; chip.onclick = () => openTokenMenu(chip); const spacer = document.createTextNode("\u200B"); const range = window.getSelection()?.rangeCount && editable.contains(window.getSelection().anchorNode) ? window.getSelection().getRangeAt(0) : null; if (range) { range.insertNode(spacer); range.insertNode(chip); range.setStart(spacer, 1); range.collapse(true); window.getSelection().removeAllRanges(); window.getSelection().addRange(range); } else editable.append(chip, spacer); valueMenu.hidden = true; save(); refreshPreview(); }; valueMenu.appendChild(option); });
  values.onclick = event => { event.stopPropagation(); const opening = valueMenu.hidden; valueMenu.hidden = !opening; if (opening) { const anchor = values.getBoundingClientRect(), menu = valueMenu.getBoundingClientRect(); valueMenu.style.left = `${Math.max(12, Math.min(anchor.left, window.innerWidth - menu.width - 12))}px`; valueMenu.style.top = `${Math.max(12, anchor.top - menu.height - 4)}px`; } }; const formatSelection = command => { const range = window.getSelection()?.rangeCount ? window.getSelection().getRangeAt(0) : null; let changedToken = false; if (range && editable.contains(range.commonAncestorContainer)) editable.querySelectorAll(".timeline-format-token").forEach(chip => { if (!range.intersectsNode(chip)) return; const key = command === "bold" ? "bold" : "italic"; chip.dataset[key] = String(chip.dataset[key] !== "true"); chip.classList.toggle(`is-${key}`, chip.dataset[key] === "true"); changedToken = true; }); editable.focus(); if (!changedToken || range?.toString()) document.execCommand(command); save(); refreshPreview(); };
  const bold = document.createElement("button"); bold.type = "button"; bold.textContent = "B"; bold.className = "timeline-format-style"; bold.onclick = () => formatSelection("bold"); const italic = document.createElement("button"); italic.type = "button"; italic.textContent = "I"; italic.className = "timeline-format-style is-italic"; italic.onclick = () => formatSelection("italic");
  actions.append(values, valueMenu, bold, italic); const preview = document.createElement("div"); preview.className = "timeline-format-preview"; preview.append("Aperçu : "); const sample = document.createElement("span"); preview.appendChild(sample);
  const refreshPreview = () => { sample.replaceChildren(); formatExampleParts(level).forEach(part => { const node = document.createElement(part.bold ? "strong" : part.italic ? "em" : "span"); node.textContent = part.value; sample.appendChild(node); }); };
  const originalSave = save; editable.oninput = () => { originalSave(); refreshPreview(); }; renderContent(); refreshPreview(); wrap.append(caption, editable, actions, preview); return wrap;
}
...
function renderThemeEditor() {
  ...
  card.append(section("Nuancier", [note, preset, groups], false));
  ...
  const selectedTimelineLevel = timelineLevels[0];
  const levelPicker = input("Niveau à modifier", "theme-timeline-level", selectedTimelineLevel.key, "select", timelineLevels.map(level => [level.key, level.label]));
  const levelFields = document.createElement("div");
  const renderTimelineLevelFields = () => {
    const level = timelineLevels.find(candidate => candidate.key === levelPicker.querySelector("select").value) || selectedTimelineLevel;
    const mainColor = colorPicker("Couleur principale", themeAppearance(level.fillRole), value => updateThemeAppearance(level.fillRole, value));
    const alternateColor = colorPicker("Couleur alternée", themeAppearance(level.altRole), value => updateThemeAppearance(level.altRole, value));
    const height = input("Hauteur de ligne (12 à 120 px)", `theme-${level.key}-height`, planningData.layout[`${level.key}Height`], "number");
    const heightInput = height.querySelector("input"); heightInput.min = "12"; heightInput.max = "120"; heightInput.step = "1"; heightInput.oninput = event => updateTimelineLevelStyle(level.key, "height", event.target.value);
    const fontSize = input("Taille de police (6 à 48 px)", `theme-${level.key}-font-size`, planningData.theme.timeline[level.key], "number");
    const fontSizeInput = fontSize.querySelector("input"); fontSizeInput.min = "6"; fontSizeInput.max = "48"; fontSizeInput.step = "1"; fontSizeInput.oninput = event => updateTimelineLevelStyle(level.key, "fontSize", event.target.value);
    const format = timelineFormatEditor(level.key);
    const presets = TIMELINE_FORMAT_PRESETS[level.key];
    if (presets?.length) { const presetRow = document.createElement("div"); presetRow.className = "timeline-format-presets"; presetRow.append(document.createTextNode("Formats rapides")); presets.forEach(([label, parts]) => { const button = document.createElement("button"); button.type = "button"; button.textContent = label; button.onclick = () => { updateTimelineFormat(level.key, clone(parts)); renderThemeEditor(); }; presetRow.appendChild(button); }); format.appendChild(presetRow); }
    levelFields.replaceChildren(fieldGrid(mainColor, alternateColor), fieldGrid(height, fontSize), format);
  };
  levelPicker.querySelector("select").onchange = renderTimelineLevelFields;
  renderTimelineLevelFields();
  const timelineNote = document.createElement("p"); timelineNote.className = "hint"; timelineNote.textContent = "Choisissez un niveau pour régler ses couleurs, sa hauteur et sa taille de police.";
  card.append(section("Frise temporelle", [timelineLocaleControl, timelineNote, levelPicker, levelFields], false));
  const lineFields = [["todayLine", "Ligne d’aujourd’hui"], ["dependencyIncoming", "Dépendance entrante"], ["dependencyOutgoing", "Dépendance sortante"], ["dependencyNeutral", "Dépendance neutre"]].map(([key, label]) => colorPicker(label, themeAppearance(key), value => updateThemeAppearance(key, value)));
  const lineNote = document.createElement("p"); lineNote.className = "hint"; lineNote.textContent = "Choisissez une couleur du nuancier ou une couleur personnalisée pour les lignes et les flèches.";
  card.append(section("Lignes et flèches", [lineNote, fieldGrid(...lineFields)], false));
  editor.appendChild(card); layout();
}
...
document.addEventListener("click", event => { if (!event.target.closest(".timeline-format-actions, .timeline-value-menu")) document.querySelectorAll(".timeline-value-menu").forEach(menu => menu.hidden = true); if (!event.target.closest(".timeline-format-token, .timeline-token-menu")) document.querySelectorAll(".timeline-token-menu").forEach(menu => menu.remove()); });

...
function emptyPlanning() {
  ...
  return {
    ...
    theme: { preset: "ocean", colors: clone(THEME_PRESETS.ocean.colors), appearance: clone(DEFAULT_THEME_APPEARANCE), timeline: clone(DEFAULT_TIMELINE_FONT_SIZES) },
...
function normalise(data) {
  ...
  data.theme = { preset: presetId, colors, appearance: { ...DEFAULT_THEME_APPEARANCE, ...appearance }, timeline: { ...DEFAULT_TIMELINE_FONT_SIZES, ...(data.theme?.timeline || {}) }, grid: { primary: { ...DEFAULT_THEME_GRID.primary, ...(data.theme?.grid?.primary || {}), ...(data.theme?.grid?.primary?.color ? {} : legacyGridColor ? { color: legacyGridColor } : {}) }, secondary: { ...DEFAULT_THEME_GRID.secondary, ...(data.theme?.grid?.secondary || {}) } } };
...
function renderThemeEditor() {
  ...
  const renderTimelineLevelFields = () => {
    ...
    levelFields.replaceChildren(fieldGrid(mainColor, alternateColor), fieldGrid(height, fontSize));
  };
